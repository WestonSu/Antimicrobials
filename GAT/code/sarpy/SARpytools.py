...
class Fragment:
    ...
    _nhString = '[nH]'
    _nh = pybel.Smarts(_nhString)
    _biphenylfp = set(pybel.readstring('smi', 'c1ccccc1c1ccccc1').calcfp().bits)
    ...
    def __init__(self, smilesFrag):
        ...
        self.mol = pybel.readstring('smi', self._molSmiles)
...
class Loader:
    ...
    def _readsdf(self, _filter = None):
        ...
        for mol in pybel.readfile('sdf', self.path):
            if mol.atoms:
                data = mol.data
    ...
    def _readcsv(self, _filter = None):
        ...
        for row in reader(open(self.path)):
            ...
            if _filter and not _filter.evaluate(data):
                continue
            try:
                mol = pybel.readstring('smi', data[self.smilesHeader])
